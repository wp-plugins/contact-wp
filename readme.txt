=== Plugin Name ===
Contributors: Sonicity Plugins
Donate link: http://plugins.sonicity.eu
Tags: contact, form, user, email
Requires at least: 2.0.2
Tested up to: 3.1.2
Stable tag: 1.0.7

This Contact Form plugin allows you to create a simple feedback submission form.

== Description ==

This Contact Form plugin allows you to create a simple feedback submission form, which let's the user send a message to you, securely, quickly and easily.

It generates multiple text fields, which can be edited from the Admin Panel. This let's you add extra fields, such as a phone number and a message subject.

This plugin is very simple to use - Simply enable the plugin, enter your e-mail address and you'll have a simple contact form to be used on your blog!


== Installation ==

1. Upload `/contact-wp/` to the `/wp-content/plugins/` directory
2. Activate the plugin through the 'Plugins' menu in WordPress
3. Edit the Contact Settings in your Administration Panel
4. Add the tag [contact] to your desired page, in HTML view.

== Frequently Asked Questions ==

= Does this work? =

Yes.

== Changelog ==

= 1.0.7 =
* Removed images for CAPTCHA - lower file sizes
* A bit more space for the CAPTCHA and the box

= 1.0.6 =
* Cleaner admin panel - checkboxes
* Removed required field text
* Re-aligned CAPTCHA

= 1.0.5 =
* Secured the CAPTCHA
* More random CAPTCHA

= 1.0.4 =
* Improved settings
* How to use plugin in admin panel
* Required field notice on plugin
* CAPTCHA backgrounds

= 1.0.3 =
* Contact Title

= 1.0.2 =
* Easier CAPTCHA admin options

= 1.0.1 =
* Fixed CAPTCHA code
* Altered admin settings

= 1.0 =
* Initial Release
